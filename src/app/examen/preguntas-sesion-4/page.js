...
import { PREGUNTAS_SESION4 } from "../../../data/examen-preguntas-sesion-4";

const TITULO_EXAMEN = "Preguntas Sesión 4";
const CONTRASENA = "1111";

export default function ExamenPreguntasSesion4Page() {
  const router = useRouter();
  const [autenticado, setAutenticado] = useState(false);
  const [contrasenaInput, setContrasenaInput] = useState("");
  const [errorClave, setErrorClave] = useState(false);

  const [respuestas, setRespuestas] = useState(Array(PREGUNTAS_SESION4.length).fill(null));
  ...
  const todasRespondidas = totalRespondidas === PREGUNTAS_SESION4.length;

  const aciertos = useMemo(() => {
    if (!finalizado) return 0;
    return respuestas.filter((r, i) => r === PREGUNTAS_SESION4[i].correcta).length;
  ...
  function reiniciar() {
    setRespuestas(Array(PREGUNTAS_SESION4.length).fill(null));
    setFinalizado(false);
    setConfirmando(false);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
      ...
      <header className="bg-gradient-to-r from-orange-500 via-orange-600 to-orange-700 text-white py-8 px-4 shadow-lg">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-orange-100 text-sm sm:text-base">
            🛡️ SNTE Sección 21 · {PREGUNTAS_SESION4.length} reactivos LGE + Plan 2022 NEM
          </p>
          ...
          <div className="bg-white border-2 border-orange-400 rounded-2xl p-6 shadow-xl">
            <div className="text-center space-y-3">
              <div className="text-5xl">
                {aciertos >= PREGUNTAS_SESION4.length * 0.8
                  ? "🏆"
                  : aciertos >= PREGUNTAS_SESION4.length * 0.5
                  ? "🎉"
                  : "📚"}
              </div>
              <h2 className="text-3xl font-bold text-slate-800">
                {aciertos} / {PREGUNTAS_SESION4.length} aciertos
              </h2>
              <p className="text-sm text-slate-600">
                {aciertos >= PREGUNTAS_SESION4.length * 0.8
                  ? "¡Excelente! Dominas los temas de la sesión 4."
                  : aciertos >= PREGUNTAS_SESION4.length * 0.5
          ...
          <div className="bg-white border border-orange-200 rounded-xl p-4 sticky top-4 z-10 shadow-md">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-slate-700">
                📋 Progreso: {totalRespondidas} / {PREGUNTAS_SESION4.length} respondidas
              </span>
              <button
                onClick={finalizar}
            ...
            <div className="mt-2 w-full bg-slate-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${(totalRespondidas / PREGUNTAS_SESION4.length) * 100}%` }}
              />
            </div>
          </div>
        )}

        {PREGUNTAS_SESION4.map((p, idxP) => {
              ...
              {p.tipo === "relacionar" && p.columnaA && p.columnaB && (
                <div className="grid sm:grid-cols-2 gap-3 mb-4 ml-12">
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                    <p className="text-xs font-bold text-orange-700 mb-2">📜 REDACCIONES:</p>
                    {p.columnaA.map((c, i) => (
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <p className="text-xs font-bold text-amber-700 mb-2">🏷️ TIPOS:</p>
                    {p.columnaB.map((c, i) => (
                      <p key={i} className="text-sm text-slate-700 mb-2 leading-snug">
                        {c}
                      </p>
          ...
          <div className="bg-white border-2 border-orange-300 rounded-2xl p-5 text-center sticky bottom-4 shadow-xl">
            ...
            >
              {todasRespondidas
                ...
                : `📝 Respondidas ${totalRespondidas}/${PREGUNTAS_SESION4.length} — Finalizar de todos modos`}
            </button>
          </div>
        )}
        ...
        {confirmando && !todasRespondidas && (
          <div className="bg-orange-100 border-2 border-orange-400 rounded-xl p-4">
            <p className="font-bold text-orange-900">
              ⚠️ Te faltan {PREGUNTAS_SESION4.length - totalRespondidas} preguntas por responder.
            </p>
